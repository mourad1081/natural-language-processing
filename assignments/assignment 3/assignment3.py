        ...
        if os.path.isfile(path_file):
            ...
            with open("bigrams.txt") as file:
                for line in file:
                    ...
                    self.vocabulary[bigram[1]] += float(bigram[0])
                    self.vocabulary[bigram[2]] += float(bigram[0])
    ...
    def get_perplexity_from(self, text):
        ...
        while end < len(txt):
            ...
            if txt[start] not in self.bigrams:
                print(self.k, len(self.vocabulary))
...
def render(file):
    ...
    # Uncomment if you want to generate a VERY LONG sentence
    # and compute its perplexity
    # parser.generate_random_output(length=65000)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        render("example_constraint_5.txt")
